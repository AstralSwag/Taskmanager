...
if [ "$SUB_EXISTS" = "1" ]; then
  ...
  # Создаём подписку
  CONN_STR="host=n8n.it4retail.tech port=$PLANE_PORT user=$REPLICATOR_USER password=$REPLICATOR_PASSWORD dbname=$PLANE_DB sslmode=require sslrootcert=/etc/ssl/postgresql/root.crt"
