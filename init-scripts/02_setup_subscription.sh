...
if [ "$SUB_EXISTS" = "1" ]; then
  ...
  # Создаём подписку
  CONN_STR="host=$PLANE_HOST port=$PLANE_PORT user=$REPLICATOR_USER password=$REPLICATOR_PASSWORD dbname=$PLANE_DB sslmode=require sslrootcert=/etc/ssl/postgresql/root.crt sslsni=0"
